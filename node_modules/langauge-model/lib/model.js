...
var Storage = Interface.define("Storage", { 
  ...
  save( p = this.p,m = this.m,fs = this.fs ){ 
      ...
      return fs.insert(p).then((nil) => {
        ...
        return (function(sequences, counts) {
          /* ../../node_modules/kit/inc/macros.sibilant:162:9 */
        
          return Promise.all([ fs.set(Path.join(p, "counts.json"), JSON.stringify(counts)), fs.set(Path.join(p, "sequences.json"), JSON.stringify(sequences)) ]);
        })(m._nodes.map(($fpipe) => {
  ...
  load( p = this.p,fs = this.fs ){ 
      ...
      var m = create(Model)();
      return Promise.all([ fs.find(Path.join(p, "counts.json")).then(($fpipe) => {
      	
        return $fpipe.string;
      
      }).then(JSON.parse), fs.find(Path.join(p, "sequences.json")).then(($fpipe) => {
      	
        return $fpipe.string;
      
      }).then(JSON.parse) ]).then(([ counts, sequences ]) => {
      	
        return sequences.reduce((m, s, i) => {
...
var Model = Interface.define("Model", { 
  ...
  add( seq = this.seq,t = 1 ){ 
      ...
      (function() {
        if (node.value) {
          return node.value.count = (node.count + t);
        } else {
          node.value = { 
            seq,
            count:t
